...
def headless(options):  #  [Temporary Function] -> Set Options (selenium) to headless mode
...
del Popen, getuser, Options, headless
...
def grabbing(EPISODES):  #  Grabs path and direct download link for each episode
    ...
    for path, dl in zip([PATH % ep.rsplit("-", 1)[1] for ep in EPISODES], [grabdl(VIDSTREAM + grabkey(soup)) for soup in getjs(Firefox(options=HEADLESS), EPISODES)]):
        yield path, dl
...
#  Determine anime and requested episodes from user
ANIME, EPS = usr(input(f'Download Path:  "{DOWNLOAD_PATH}"\n\n\t> '))
...
status(f"Beginning Download{'s' if isinstance(EPS, range) else ''}")
download(INFO)
